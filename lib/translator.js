...
// API calls, context detection, cultural notes, grammar, synonyms.
// Only imported in background.js — API key never leaves here.

export const LANGUAGES = {
  en: { name: 'English', native: 'English', code: 'en' },
  ne: { name: 'Nepali', native: 'नेपाली', code: 'ne' },
  tmg: { name: 'Tamang', native: 'तामाङ', code: 'tmg' },
};

export const LANGUAGE_PAIRS = [
  { source: 'en', target: 'ne' }, { source: 'ne', target: 'en' },
  { source: 'en', target: 'tmg' }, { source: 'tmg', target: 'en' },
  { source: 'ne', target: 'tmg' }, { source: 'tmg', target: 'ne' },
];

// page context detection based on URL
export function detectPageContext(url = '', title = '') {
  const patterns = {
    technical: [/github\.com/, /stackoverflow\.com/, /docs\./, /developer\./, /api\./, /npm\.js/],
    news: [/bbc\./, /cnn\./, /reuters\./, /nytimes\./, /nepalnews\./, /ekantipur\./, /onlinekhabar\./],
    academic: [/scholar\./, /arxiv\./, /researchgate\./, /jstor\./, /pubmed\./],
    meeting: [/meet\.google\.com/, /zoom\.us/, /teams\.microsoft\.com/],
    email: [/mail\.google\.com/, /outlook\.live\.com/],
  };
  for (const [ctx, rules] of Object.entries(patterns)) {
    for (const r of rules) if (r.test(url) || r.test(title)) return ctx;
  }
  return 'casual';
}
